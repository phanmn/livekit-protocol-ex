...
defmodule LiveKitProtocolEx.Destination do
  @moduledoc false
  ...
  defstruct region: "", country: "", city: "", __uf__: []
  ...
  (
    ...
    defp encode_country({acc, acc_size}, msg) do
      if msg.country == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.country)
    ...
    defp encode_city({acc, acc_size}, msg) do
      if msg.city == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.city)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:city, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LiveKitProtocolEx.Destination))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[city: Protox.Decode.validate_string!(delimited)], rest}
  ...
  def schema() do
    %{
      ...
      name: LiveKitProtocolEx.Destination,
      syntax: :proto3
    }
  end
end
