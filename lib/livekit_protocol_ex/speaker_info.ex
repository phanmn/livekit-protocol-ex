...
defmodule LivekitProtocolEx.SpeakerInfo do
  @moduledoc false
  ...
  defstruct active: false, level: 0.0, sid: "", __uf__: []
  ...
  (
    ...
    defp encode_active({acc, acc_size}, msg) do
      if msg.active == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.active)
        {["\x18", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:active, "invalid field value"), __STACKTRACE__
    end

    defp encode_level({acc, acc_size}, msg) do
      if msg.level == 0.0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_float(msg.level)
        {["\x15", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.SpeakerInfo))
        end
      )
    )
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.SpeakerInfo,
      syntax: :proto3
    }
  end
end
