...
defmodule LivekitProtocolEx.WebhookConfig do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          filter_params: LivekitProtocolEx.FilterParams.t() | nil,
          signing_key: String.t(),
          url: String.t(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct filter_params: nil, signing_key: "", url: "", __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_filter_params(msg)
        |> encode_signing_key(msg)
        |> encode_url(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_filter_params({acc, acc_size}, msg) do
      if msg.filter_params == nil do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_message(msg.filter_params)
        {["\x1A", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:filter_params, "invalid field value"), __STACKTRACE__
    end

    defp encode_signing_key({acc, acc_size}, msg) do
      if msg.signing_key == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.signing_key)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:signing_key, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.WebhookConfig))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
              ...
              {[
                 filter_params:
                   Protox.MergeMessage.merge(
                     msg.filter_params,
                     LivekitProtocolEx.FilterParams.decode!(delimited)
                   )
               ], rest}
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[signing_key: Protox.Decode.validate_string!(delimited)], rest}
  ...
  (
    ...
    def default(:filter_params) do
      {:ok, nil}
    end

    def default(:signing_key) do
      {:ok, ""}
    end
    ...
    %{
      ...
      fields: %{
        filter_params: %{
          ...
          name: :filter_params,
          tag: 3,
          type: {:message, LivekitProtocolEx.FilterParams}
        },
        signing_key: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.WebhookConfig,
      syntax: :proto3
    }
  end
end
