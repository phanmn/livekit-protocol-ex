...
defmodule LivekitProtocolEx.SessionDescription do
  @moduledoc false
  ...
  defstruct id: 0, sdp: "", type: "", __uf__: []
  ...
  (
    ...
    defp encode_sdp({acc, acc_size}, msg) do
      if msg.sdp == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.sdp)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:sdp, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.SessionDescription))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              ...
              {[sdp: Protox.Decode.validate_string!(delimited)], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.SessionDescription,
      syntax: :proto3
    }
  end
end
