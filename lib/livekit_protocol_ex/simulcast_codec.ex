...
defmodule LivekitProtocolEx.SimulcastCodec do
  @moduledoc false
  ...
  defstruct video_layer_mode: :MODE_UNUSED, layers: [], cid: "", codec: "", __uf__: []
  ...
  (
    ...
    defp encode_codec({acc, acc_size}, msg) do
      if msg.codec == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.codec)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:codec, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.SimulcastCodec))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[codec: Protox.Decode.validate_string!(delimited)], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.SimulcastCodec,
      syntax: :proto3
    }
  end
end
