...
defmodule LivekitProtocolEx.TrackPublishedResponse do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          track: LivekitProtocolEx.TrackInfo.t() | nil,
          cid: String.t(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct track: nil, cid: "", __uf__: []
  ...
  (
    ...
    defp encode_track({acc, acc_size}, msg) do
      if msg.track == nil do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_message(msg.track)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:track, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.TrackPublishedResponse))
        end
      )
    )
              ...
              {[
                 track:
                   Protox.MergeMessage.merge(
                     msg.track,
                     LivekitProtocolEx.TrackInfo.decode!(delimited)
                   )
               ], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.TrackPublishedResponse,
      syntax: :proto3
    }
  end
end
