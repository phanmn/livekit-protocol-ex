...
defmodule LivekitProtocolEx.SIPUri do
  @moduledoc false
  ...
  defstruct transport: :SIP_TRANSPORT_AUTO, port: 0, ip: "", host: "", user: "", __uf__: []
  ...
  (
    ...
    defp encode_port({acc, acc_size}, msg) do
      if msg.port == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.port)
        {[" ", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
    ...
    defp encode_host({acc, acc_size}, msg) do
      if msg.host == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.host)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:host, "invalid field value"), __STACKTRACE__
    end
    ...
    defp encode_user({acc, acc_size}, msg) do
      if msg.user == "" do
        ...
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.user)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.SIPUri))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[host: Protox.Decode.validate_string!(delimited)], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[user: Protox.Decode.validate_string!(delimited)], rest}
