...
defmodule LivekitProtocolEx.EncryptedPacket do
  @moduledoc false
  ...
  defstruct encrypted_value: "", key_index: 0, iv: "", encryption_type: :NONE, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_encrypted_value(msg)
        |> encode_key_index(msg)
        |> encode_iv(msg)
    ...
    defp encode_encrypted_value({acc, acc_size}, msg) do
      if msg.encrypted_value == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bytes(msg.encrypted_value)
        {["\"", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:encrypted_value, "invalid field value"), __STACKTRACE__
    end

    defp encode_key_index({acc, acc_size}, msg) do
      if msg.key_index == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.key_index)
        {["\x18", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:key_index, "invalid field value"), __STACKTRACE__
    end

    defp encode_iv({acc, acc_size}, msg) do
      if msg.iv == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bytes(msg.iv)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:iv, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.EncryptedPacket))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<4::5, _wire_type::3, bytes::binary>> ->
              ...
              {[encrypted_value: delimited], rest}

            <<3::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_uint32(bytes)
              {[key_index: value], rest}
  ...
  (
    ...
    def default(:encrypted_value) do
      {:ok, ""}
    end

    def default(:key_index) do
      {:ok, 0}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.EncryptedPacket,
      syntax: :proto3
    }
  end
end
