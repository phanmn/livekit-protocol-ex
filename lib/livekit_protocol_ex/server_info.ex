...
defmodule LivekitProtocolEx.ServerInfo do
  @moduledoc false
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_agent_protocol(msg)
        |> encode_debug_info(msg)
        |> encode_node_id(msg)
        |> encode_region(msg)
        |> encode_protocol(msg)
        |> encode_version(msg)
    ...
    defp encode_agent_protocol({acc, acc_size}, msg) do
      if msg.agent_protocol == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_int32(msg.agent_protocol)
        {["8", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:agent_protocol, "invalid field value"), __STACKTRACE__
    end

    defp encode_debug_info({acc, acc_size}, msg) do
      if msg.debug_info == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.debug_info)
        {["2", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:debug_info, "invalid field value"), __STACKTRACE__
    end
    ...
    defp encode_edition({acc, acc_size}, msg) do
      if msg.edition == :Standard do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.edition
          |> LivekitProtocolEx.ServerInfo.Edition.encode()
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:edition, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.ServerInfo))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<7::5, _wire_type::3, bytes::binary>> ->
              ...
              {[agent_protocol: value], rest}

            <<6::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[debug_info: Protox.Decode.validate_string!(delimited)], rest}
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {value, rest} =
                Protox.Decode.parse_enum(bytes, LivekitProtocolEx.ServerInfo.Edition)
  ...
  (
    ...
    def default(:agent_protocol) do
      {:ok, 0}
    end

    def default(:debug_info) do
      {:ok, ""}
    end
    ...
    %{
      ...
      fields: %{
        ...
        edition: %{
          ...
          name: :edition,
          tag: 1,
          type: {:enum, LivekitProtocolEx.ServerInfo.Edition}
        },
        node_id: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.ServerInfo,
      syntax: :proto3
    }
  end
end
