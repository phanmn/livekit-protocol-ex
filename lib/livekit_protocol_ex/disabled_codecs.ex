...
defmodule LivekitProtocolEx.DisabledCodecs do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          publish: [LivekitProtocolEx.Codec.t()],
          codecs: [LivekitProtocolEx.Codec.t()],
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct publish: [], codecs: [], __uf__: []
  ...
  (
    ...
    defp encode_publish({acc, acc_size}, msg) do
      case msg.publish do
        [] ->
          {acc, acc_size}
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:publish, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.DisabledCodecs))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              ...
              {[publish: msg.publish ++ [LivekitProtocolEx.Codec.decode!(delimited)]], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[codecs: msg.codecs ++ [LivekitProtocolEx.Codec.decode!(delimited)]], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.DisabledCodecs,
      syntax: :proto3
    }
  end
end
