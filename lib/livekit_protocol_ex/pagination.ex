...
defmodule LivekitProtocolEx.Pagination do
  @moduledoc false
  ...
  defstruct limit: 0, after_id: "", __uf__: []
  ...
  (
    ...
    defp encode_after_id({acc, acc_size}, msg) do
      if msg.after_id == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.after_id)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:after_id, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.Pagination))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              ...
              {[after_id: Protox.Decode.validate_string!(delimited)], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.Pagination,
      syntax: :proto3
    }
  end
end
