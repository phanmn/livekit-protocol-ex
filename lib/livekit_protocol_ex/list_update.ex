...
defmodule LivekitProtocolEx.ListUpdate do
  @moduledoc false
  ...
  defstruct clear: false, remove: [], add: [], set: [], __uf__: []
  ...
  (
    ...
    defp encode_clear({acc, acc_size}, msg) do
      if msg.clear == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.clear)
        {[" ", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:clear, "invalid field value"), __STACKTRACE__
    end
    ...
    defp encode_add({acc, acc_size}, msg) do
      case msg.add do
        [] ->
          {acc, acc_size}
    ...
    defp encode_set({acc, acc_size}, msg) do
      case msg.set do
        [] ->
          {acc, acc_size}
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:set, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.ListUpdate))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<3::5, _wire_type::3, bytes::binary>> ->
              ...
              {[remove: msg.remove ++ [Protox.Decode.validate_string!(delimited)]], rest}

            <<2::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[add: msg.add ++ [Protox.Decode.validate_string!(delimited)]], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[set: msg.set ++ [Protox.Decode.validate_string!(delimited)]], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.ListUpdate,
      syntax: :proto3
    }
  end
end
