...
defmodule LivekitProtocolEx.LeaveRequest do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          regions: LivekitProtocolEx.RegionSettings.t() | nil,
          action: atom(),
          reason: atom(),
  ...
  (
    (
      ...
      def encode!(msg) do
        ...
        |> encode_action(msg)
        |> encode_reason(msg)
        |> encode_can_reconnect(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_regions({acc, acc_size}, msg) do
      if msg.regions == nil do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_message(msg.regions)
        {["\"", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:regions, "invalid field value"), __STACKTRACE__
    end

    defp encode_action({acc, acc_size}, msg) do
      if msg.action == :DISCONNECT do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.action
          |> LivekitProtocolEx.LeaveRequest.Action.encode()
    ...
    defp encode_reason({acc, acc_size}, msg) do
      if msg.reason == :UNKNOWN_REASON do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.reason |> LivekitProtocolEx.DisconnectReason.encode() |> Protox.Encode.encode_enum()
    ...
    defp encode_can_reconnect({acc, acc_size}, msg) do
      if msg.can_reconnect == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.can_reconnect)
        {["\b", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:can_reconnect, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.LeaveRequest))
        end
      )
    )
              ...
              {[
                 regions:
                   Protox.MergeMessage.merge(
                     msg.regions,
                     LivekitProtocolEx.RegionSettings.decode!(delimited)
                   )
               ], rest}

            <<3::5, _wire_type::3, bytes::binary>> ->
              {value, rest} =
                Protox.Decode.parse_enum(bytes, LivekitProtocolEx.LeaveRequest.Action)
    ...
    %{
      ...
      fields: %{
        action: %{
          ...
          extender: nil,
          kind: %{__struct__: Protox.Scalar, default_value: :DISCONNECT},
          label: :optional,
          name: :action,
          tag: 3,
          type: {:enum, LivekitProtocolEx.LeaveRequest.Action}
        },
        can_reconnect: %{
          __struct__: Protox.Field,
          extender: nil,
        ...
        regions: %{
          ...
          name: :regions,
          tag: 4,
          type: {:message, LivekitProtocolEx.RegionSettings}
        }
      },
      file_options: %{
      ...
      name: LivekitProtocolEx.LeaveRequest,
      syntax: :proto3
    }
  end
end
