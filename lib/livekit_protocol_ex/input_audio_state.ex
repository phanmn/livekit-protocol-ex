...
defmodule LivekitProtocolEx.InputAudioState do
  @moduledoc false
  ...
  defstruct sample_rate: 0, channels: 0, average_bitrate: 0, mime_type: "", __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_sample_rate(msg)
        |> encode_channels(msg)
        |> encode_average_bitrate(msg)
        |> encode_mime_type(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_sample_rate({acc, acc_size}, msg) do
      if msg.sample_rate == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.sample_rate)
        {[" ", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:sample_rate, "invalid field value"), __STACKTRACE__
    end

    defp encode_channels({acc, acc_size}, msg) do
      if msg.channels == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.channels)
    ...
    defp encode_average_bitrate({acc, acc_size}, msg) do
      if msg.average_bitrate == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.average_bitrate)
        {["\x10", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:average_bitrate, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.InputAudioState))
        end
      )
    )
  ...
  (
    ...
    def default(:average_bitrate) do
      {:ok, 0}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.InputAudioState,
      syntax: :proto3
    }
  end
end
