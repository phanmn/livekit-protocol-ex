...
defmodule LivekitProtocolEx.Pong do
  @moduledoc false
  ...
  defstruct timestamp: 0, last_ping_timestamp: 0, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        ...
        |> encode_last_ping_timestamp(msg)
    ...
    defp encode_last_ping_timestamp({acc, acc_size}, msg) do
      if msg.last_ping_timestamp == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_int64(msg.last_ping_timestamp)
        {["\b", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:last_ping_timestamp, "invalid field value"),
                __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.Pong))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_int64(bytes)
              {[last_ping_timestamp: value], rest}
  ...
  (
    ...
    def default(:last_ping_timestamp) do
      {:ok, 0}
    end
