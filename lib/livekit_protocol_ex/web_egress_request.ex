...
defmodule LivekitProtocolEx.WebEgressRequest do
  @moduledoc false
  ...
  (
    ...
    defp encode_await_start_signal({acc, acc_size}, msg) do
      if msg.await_start_signal == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.await_start_signal)
        {["`", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:await_start_signal, "invalid field value"),
                __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.WebEgressRequest))
        end
      )
    )
  ...
  (
    ...
    def default(:await_start_signal) do
      {:ok, false}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.WebEgressRequest,
      syntax: :proto3
    }
  end
end
