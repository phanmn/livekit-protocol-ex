...
defmodule LivekitProtocolEx.SubscriptionPermission do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          track_permissions: [LivekitProtocolEx.TrackPermission.t()],
          all_participants: boolean(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct track_permissions: [], all_participants: false, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_track_permissions(msg)
        |> encode_all_participants(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_track_permissions({acc, acc_size}, msg) do
      case msg.track_permissions do
        [] ->
          {acc, acc_size}
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:track_permissions, "invalid field value"),
                __STACKTRACE__
    end

    defp encode_all_participants({acc, acc_size}, msg) do
      if msg.all_participants == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.all_participants)
        {["\b", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:all_participants, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.SubscriptionPermission))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              ...
              {[
                 track_permissions:
                   msg.track_permissions ++ [LivekitProtocolEx.TrackPermission.decode!(delimited)]
               ], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_bool(bytes)
              {[all_participants: value], rest}
  ...
  (
    ...
    def default(:track_permissions) do
      {:error, :no_default_value}
    end

    def default(:all_participants) do
      {:ok, false}
    end
    ...
    %{
      ...
      fields: %{
        ...
        track_permissions: %{
          ...
          tag: 2,
          type: {:message, LivekitProtocolEx.TrackPermission}
        }
      },
      file_options: %{
      ...
      name: LivekitProtocolEx.SubscriptionPermission,
      syntax: :proto3
    }
  end
end
