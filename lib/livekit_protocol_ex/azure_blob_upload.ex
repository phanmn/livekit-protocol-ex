...
defmodule LivekitProtocolEx.AzureBlobUpload do
  @moduledoc false
  ...
  defstruct container_name: "", account_key: "", account_name: "", __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_container_name(msg)
        |> encode_account_key(msg)
        |> encode_account_name(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_container_name({acc, acc_size}, msg) do
      if msg.container_name == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.container_name)
        {["\x1A", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:container_name, "invalid field value"), __STACKTRACE__
    end

    defp encode_account_key({acc, acc_size}, msg) do
      if msg.account_key == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.account_key)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:account_key, "invalid field value"), __STACKTRACE__
    end

    defp encode_account_name({acc, acc_size}, msg) do
      if msg.account_name == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.account_name)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:account_name, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.AzureBlobUpload))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<3::5, _wire_type::3, bytes::binary>> ->
              ...
              {[container_name: Protox.Decode.validate_string!(delimited)], rest}

            <<2::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[account_key: Protox.Decode.validate_string!(delimited)], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[account_name: Protox.Decode.validate_string!(delimited)], rest}
  ...
  (
    ...
    def default(:container_name) do
      {:ok, ""}
    end

    def default(:account_key) do
      {:ok, ""}
    end

    def default(:account_name) do
      {:ok, ""}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.AzureBlobUpload,
      syntax: :proto3
    }
  end
end
