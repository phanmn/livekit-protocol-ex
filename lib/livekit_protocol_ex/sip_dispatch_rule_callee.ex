...
defmodule LivekitProtocolEx.SIPDispatchRuleCallee do
  @moduledoc false
  ...
  defstruct randomize: false, pin: "", room_prefix: "", __uf__: []
  ...
  (
    ...
    defp encode_randomize({acc, acc_size}, msg) do
      if msg.randomize == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.randomize)
        {["\x18", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:randomize, "invalid field value"), __STACKTRACE__
    end
    ...
    defp encode_room_prefix({acc, acc_size}, msg) do
      if msg.room_prefix == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.room_prefix)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:room_prefix, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.SIPDispatchRuleCallee))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[room_prefix: Protox.Decode.validate_string!(delimited)], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.SIPDispatchRuleCallee,
      syntax: :proto3
    }
  end
end
