...
defmodule LivekitProtocolEx.WorkerPong do
  @moduledoc false
  ...
  defstruct timestamp: 0, last_timestamp: 0, __uf__: []
  ...
  (
    ...
    defp encode_last_timestamp({acc, acc_size}, msg) do
      if msg.last_timestamp == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_int64(msg.last_timestamp)
        {["\b", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:last_timestamp, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.WorkerPong))
        end
      )
    )
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.WorkerPong,
      syntax: :proto3
    }
  end
end
