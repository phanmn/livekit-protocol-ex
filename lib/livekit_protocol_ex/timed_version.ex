...
defmodule LivekitProtocolEx.TimedVersion do
  @moduledoc false
  ...
  defstruct ticks: 0, unix_micro: 0, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_ticks(msg)
        |> encode_unix_micro(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_ticks({acc, acc_size}, msg) do
      if msg.ticks == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_int32(msg.ticks)
        {["\x10", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:ticks, "invalid field value"), __STACKTRACE__
    end

    defp encode_unix_micro({acc, acc_size}, msg) do
      if msg.unix_micro == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_int64(msg.unix_micro)
        {["\b", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:unix_micro, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.TimedVersion))
        end
      )
    )
  ...
  (
    ...
    def default(:ticks) do
      {:ok, 0}
    end

    def default(:unix_micro) do
      {:ok, 0}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.TimedVersion,
      syntax: :proto3
    }
  end
end
