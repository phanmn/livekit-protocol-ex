...
defmodule LivekitProtocolEx.TrickleRequest do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          final: boolean(),
          target: atom(),
          candidateInit: String.t(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct final: false, target: :PUBLISHER, candidateInit: "", __uf__: []
  ...
  (
    ...
    defp encode_target({acc, acc_size}, msg) do
      if msg.target == :PUBLISHER do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.target |> LivekitProtocolEx.SignalTarget.encode() |> Protox.Encode.encode_enum()
    ...
    defp encode_candidateInit({acc, acc_size}, msg) do
      if msg.candidateInit == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.candidateInit)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:candidateInit, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.TrickleRequest))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_enum(bytes, LivekitProtocolEx.SignalTarget)
              {[target: value], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[candidateInit: Protox.Decode.validate_string!(delimited)], rest}
    ...
    %{
      ...
      fields: %{
        ...
        target: %{
          __struct__: Protox.Field,
          extender: nil,
          kind: %{__struct__: Protox.Scalar, default_value: :PUBLISHER},
          label: :optional,
          name: :target,
          tag: 2,
          type: {:enum, LivekitProtocolEx.SignalTarget}
        }
      },
      file_options: %{
      ...
      name: LivekitProtocolEx.TrickleRequest,
      syntax: :proto3
    }
  end
end
