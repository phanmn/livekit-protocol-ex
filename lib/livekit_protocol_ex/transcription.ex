...
defmodule LivekitProtocolEx.Transcription do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          segments: [LivekitProtocolEx.TranscriptionSegment.t()],
          track_id: String.t(),
          transcribed_participant_identity: String.t(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct segments: [], track_id: "", transcribed_participant_identity: "", __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        ...
        |> encode_segments(msg)
        |> encode_track_id(msg)
        |> encode_transcribed_participant_identity(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_segments({acc, acc_size}, msg) do
    ...
    defp encode_track_id({acc, acc_size}, msg) do
      if msg.track_id == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.track_id)
        {["\x1A", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:track_id, "invalid field value"), __STACKTRACE__
    end

    defp encode_transcribed_participant_identity({acc, acc_size}, msg) do
      if msg.transcribed_participant_identity == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          Protox.Encode.encode_string(msg.transcribed_participant_identity)
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.Transcription))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<4::5, _wire_type::3, bytes::binary>> ->
              ...
              {[
                 segments:
                   msg.segments ++ [LivekitProtocolEx.TranscriptionSegment.decode!(delimited)]
               ], rest}

            <<3::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[track_id: Protox.Decode.validate_string!(delimited)], rest}

            <<2::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)

              {[transcribed_participant_identity: Protox.Decode.validate_string!(delimited)],
               rest}
  ...
  (
    ...
    def default(:track_id) do
      {:ok, ""}
    end

    def default(:transcribed_participant_identity) do
      {:ok, ""}
    end
    ...
    %{
      ...
      fields: %{
        segments: %{
          ...
          name: :segments,
          tag: 4,
          type: {:message, LivekitProtocolEx.TranscriptionSegment}
        },
        track_id: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.Transcription,
      syntax: :proto3
    }
  end
end
