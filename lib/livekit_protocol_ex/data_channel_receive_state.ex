...
defmodule LivekitProtocolEx.DataChannelReceiveState do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          ...
          publisher_sid: String.t(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct last_seq: 0, publisher_sid: "", __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_last_seq(msg)
        |> encode_publisher_sid(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_last_seq({acc, acc_size}, msg) do
      if msg.last_seq == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.last_seq)
        {["\x10", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:last_seq, "invalid field value"), __STACKTRACE__
    end

    defp encode_publisher_sid({acc, acc_size}, msg) do
      if msg.publisher_sid == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.publisher_sid)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:publisher_sid, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.DataChannelReceiveState))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              ...
              {[publisher_sid: Protox.Decode.validate_string!(delimited)], rest}
  ...
  (
    ...
    def default(:last_seq) do
      {:ok, 0}
    end

    def default(:publisher_sid) do
      {:ok, ""}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.DataChannelReceiveState,
      syntax: :proto3
    }
  end
end
