...
defmodule LivekitProtocolEx.AvailabilityRequest do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          resuming: boolean(),
          job: LivekitProtocolEx.Job.t() | nil,
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct resuming: false, job: nil, __uf__: []
  ...
  (
    ...
    defp encode_resuming({acc, acc_size}, msg) do
      if msg.resuming == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.resuming)
        {["\x10", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:resuming, "invalid field value"), __STACKTRACE__
    end

    defp encode_job({acc, acc_size}, msg) do
      if msg.job == nil do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_message(msg.job)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:job, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.AvailabilityRequest))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              ...
              {[
                 job: Protox.MergeMessage.merge(msg.job, LivekitProtocolEx.Job.decode!(delimited))
               ], rest}
    ...
    %{
      ...
      fields: %{
        job: %{
          ...
          name: :job,
          tag: 1,
          type: {:message, LivekitProtocolEx.Job}
        },
        resuming: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.AvailabilityRequest,
      syntax: :proto3
    }
  end
end
