...
defmodule LivekitProtocolEx.ImagesInfo do
  @moduledoc false
  ...
  defstruct filename_prefix: "", ended_at: 0, started_at: 0, image_count: 0, __uf__: []
  ...
  (
    ...
    defp encode_image_count({acc, acc_size}, msg) do
      if msg.image_count == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_int64(msg.image_count)
        {["\b", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:image_count, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.ImagesInfo))
        end
      )
    )
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.ImagesInfo,
      syntax: :proto3
    }
  end
end
