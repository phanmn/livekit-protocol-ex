...
defmodule LivekitProtocolEx.MediaSectionsRequirement do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          num_videos: non_neg_integer(),
          num_audios: non_neg_integer(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct num_videos: 0, num_audios: 0, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_num_videos(msg)
        |> encode_num_audios(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_num_videos({acc, acc_size}, msg) do
      if msg.num_videos == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.num_videos)
        {["\x10", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:num_videos, "invalid field value"), __STACKTRACE__
    end

    defp encode_num_audios({acc, acc_size}, msg) do
      if msg.num_audios == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.num_audios)
        {["\b", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:num_audios, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.MediaSectionsRequirement))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              ...
              {[num_videos: value], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_uint32(bytes)
              {[num_audios: value], rest}
  ...
  (
    ...
    def default(:num_videos) do
      {:ok, 0}
    end

    def default(:num_audios) do
      {:ok, 0}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.MediaSectionsRequirement,
      syntax: :proto3
    }
  end
end
