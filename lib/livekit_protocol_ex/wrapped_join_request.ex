...
defmodule LivekitProtocolEx.WrappedJoinRequest do
  @moduledoc false
  ...
  defstruct join_request: "", compression: :NONE, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_join_request(msg)
        |> encode_compression(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_join_request({acc, acc_size}, msg) do
      if msg.join_request == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bytes(msg.join_request)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:join_request, "invalid field value"), __STACKTRACE__
    end

    defp encode_compression({acc, acc_size}, msg) do
      if msg.compression == :NONE do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.compression
          |> LivekitProtocolEx.WrappedJoinRequest.Compression.encode()
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.WrappedJoinRequest))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              ...
              {[join_request: delimited], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {value, rest} =
                Protox.Decode.parse_enum(bytes, LivekitProtocolEx.WrappedJoinRequest.Compression)
    ...
    %{
      ...
      fields: %{
        compression: %{
          ...
          name: :compression,
          tag: 1,
          type: {:enum, LivekitProtocolEx.WrappedJoinRequest.Compression}
        },
        join_request: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.WrappedJoinRequest,
      syntax: :proto3
    }
  end
end
