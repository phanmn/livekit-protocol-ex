...
defmodule LivekitProtocolEx.DataStream.TextHeader do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          generated: boolean(),
          attached_stream_ids: [String.t()],
          reply_to_stream_id: String.t(),
          version: integer(),
          operation_type: atom(),
  ...
  (
    (
      ...
      def encode!(msg) do
        ...
        |> encode_attached_stream_ids(msg)
        |> encode_reply_to_stream_id(msg)
        |> encode_version(msg)
        |> encode_operation_type(msg)
    ...
    defp encode_attached_stream_ids({acc, acc_size}, msg) do
      case msg.attached_stream_ids do
        [] ->
          {acc, acc_size}
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:attached_stream_ids, "invalid field value"),
                __STACKTRACE__
    end

    defp encode_reply_to_stream_id({acc, acc_size}, msg) do
      if msg.reply_to_stream_id == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.reply_to_stream_id)
        {["\x1A", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:reply_to_stream_id, "invalid field value"),
                __STACKTRACE__
    end
    ...
    defp encode_operation_type({acc, acc_size}, msg) do
      if msg.operation_type == :CREATE do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.operation_type
          |> LivekitProtocolEx.DataStream.OperationType.encode()
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:operation_type, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.DataStream.TextHeader))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<4::5, _wire_type::3, bytes::binary>> ->
              ...
              {[
                 attached_stream_ids:
                   msg.attached_stream_ids ++ [Protox.Decode.validate_string!(delimited)]
               ], rest}

            <<3::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[reply_to_stream_id: Protox.Decode.validate_string!(delimited)], rest}
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {value, rest} =
                Protox.Decode.parse_enum(bytes, LivekitProtocolEx.DataStream.OperationType)

              {[operation_type: value], rest}
  ...
  (
    ...
    def default(:attached_stream_ids) do
      {:error, :no_default_value}
    end

    def default(:reply_to_stream_id) do
      {:ok, ""}
    end
    ...
    %{
      ...
      fields: %{
        ...
        operation_type: %{
          __struct__: Protox.Field,
          extender: nil,
          kind: %{__struct__: Protox.Scalar, default_value: :CREATE},
          label: :optional,
          name: :operation_type,
          tag: 1,
          type: {:enum, LivekitProtocolEx.DataStream.OperationType}
        },
        reply_to_stream_id: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.DataStream.TextHeader,
      syntax: :proto3
    }
  end
end
