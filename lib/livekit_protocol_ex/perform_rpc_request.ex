...
defmodule LivekitProtocolEx.PerformRpcRequest do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          response_timeout_ms: non_neg_integer(),
          payload: String.t(),
          method: String.t(),
          destination_identity: String.t(),
          room: String.t(),
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct response_timeout_ms: 0,
            payload: "",
            method: "",
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_response_timeout_ms(msg)
        |> encode_payload(msg)
        |> encode_method(msg)
        |> encode_destination_identity(msg)
        |> encode_room(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_response_timeout_ms({acc, acc_size}, msg) do
      if msg.response_timeout_ms == 0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_uint32(msg.response_timeout_ms)
        {["(", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:response_timeout_ms, "invalid field value"),
                __STACKTRACE__
    end
    ...
    defp encode_method({acc, acc_size}, msg) do
      if msg.method == "" do
        ...
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.method)
    ...
    defp encode_destination_identity({acc, acc_size}, msg) do
      if msg.destination_identity == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.destination_identity)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:destination_identity, "invalid field value"),
                __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.PerformRpcRequest))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<5::5, _wire_type::3, bytes::binary>> ->
              ...
              {[response_timeout_ms: value], rest}
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[destination_identity: Protox.Decode.validate_string!(delimited)], rest}
  ...
  (
    ...
    def default(:response_timeout_ms) do
      {:ok, 0}
    end
    ...
    def default(:destination_identity) do
      {:ok, ""}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.PerformRpcRequest,
      syntax: :proto3
    }
  end
end
