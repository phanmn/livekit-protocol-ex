...
defmodule LivekitProtocolEx.Rpc.UpdateMetricsRequest do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          max_cpu_usage: float(),
          avg_cpu_usage: float(),
          info: LivekitProtocolEx.EgressInfo.t() | nil,
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct max_cpu_usage: 0.0, avg_cpu_usage: 0.0, info: nil, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_max_cpu_usage(msg)
        |> encode_avg_cpu_usage(msg)
        |> encode_info(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_max_cpu_usage({acc, acc_size}, msg) do
      if msg.max_cpu_usage == 0.0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_float(msg.max_cpu_usage)
        {["%", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:max_cpu_usage, "invalid field value"), __STACKTRACE__
    end

    defp encode_avg_cpu_usage({acc, acc_size}, msg) do
      if msg.avg_cpu_usage == 0.0 do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_float(msg.avg_cpu_usage)
        {["\x1D", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:avg_cpu_usage, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.Rpc.UpdateMetricsRequest))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<4::5, _wire_type::3, bytes::binary>> ->
              ...
              {[max_cpu_usage: value], rest}

            <<3::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_float(bytes)
              {[avg_cpu_usage: value], rest}
              ...
              {[
                 info:
                   Protox.MergeMessage.merge(
                     msg.info,
                     LivekitProtocolEx.EgressInfo.decode!(delimited)
                   )
               ], rest}
  ...
  (
    ...
    def default(:max_cpu_usage) do
      {:ok, 0.0}
    end

    def default(:avg_cpu_usage) do
      {:ok, 0.0}
    end
    ...
    %{
      ...
      fields: %{
        ...
        info: %{
          ...
          name: :info,
          tag: 1,
          type: {:message, LivekitProtocolEx.EgressInfo}
        },
        max_cpu_usage: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.Rpc.UpdateMetricsRequest,
      syntax: :proto3
    }
  end
end
