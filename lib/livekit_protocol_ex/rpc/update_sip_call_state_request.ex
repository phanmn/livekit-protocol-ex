...
defmodule LivekitProtocolEx.Rpc.UpdateSIPCallStateRequest do
  @moduledoc false
  ...
  @type t :: %__MODULE__{
          transfer_info: LivekitProtocolEx.SIPTransferInfo.t() | nil,
          call_info: LivekitProtocolEx.SIPCallInfo.t() | nil,
          __uf__: [{non_neg_integer(), Protox.Types.tag(), binary()}]
        }
  defstruct transfer_info: nil, call_info: nil, __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_transfer_info(msg)
        |> encode_call_info(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_transfer_info({acc, acc_size}, msg) do
      if msg.transfer_info == nil do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_message(msg.transfer_info)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:transfer_info, "invalid field value"), __STACKTRACE__
    end

    defp encode_call_info({acc, acc_size}, msg) do
      if msg.call_info == nil do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_message(msg.call_info)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:call_info, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.Rpc.UpdateSIPCallStateRequest))
        end
      )
    )
              ...
              {[
                 transfer_info:
                   Protox.MergeMessage.merge(
                     msg.transfer_info,
                     LivekitProtocolEx.SIPTransferInfo.decode!(delimited)
                   )
               ], rest}
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              ...
              {[
                 call_info:
                   Protox.MergeMessage.merge(
                     msg.call_info,
                     LivekitProtocolEx.SIPCallInfo.decode!(delimited)
                   )
               ], rest}
  ...
  (
    ...
    def default(:transfer_info) do
      {:ok, nil}
    end

    def default(:call_info) do
      {:ok, nil}
    end
    ...
    %{
      ...
      fields: %{
        call_info: %{
          ...
          name: :call_info,
          tag: 1,
          type: {:message, LivekitProtocolEx.SIPCallInfo}
        },
        transfer_info: %{
          __struct__: Protox.Field,
          extender: nil,
          ...
          name: :transfer_info,
          tag: 2,
          type: {:message, LivekitProtocolEx.SIPTransferInfo}
        }
      },
      file_options: %{
      ...
      name: LivekitProtocolEx.Rpc.UpdateSIPCallStateRequest,
      syntax: :proto3
    }
  end
end
