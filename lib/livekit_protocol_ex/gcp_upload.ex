...
defmodule LivekitProtocolEx.GCPUpload do
  @moduledoc false
  ...
  defstruct proxy: nil, bucket: "", credentials: "", __uf__: []
  ...
  (
    ...
    defp encode_credentials({acc, acc_size}, msg) do
      if msg.credentials == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.credentials)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:credentials, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.GCPUpload))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[credentials: Protox.Decode.validate_string!(delimited)], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.GCPUpload,
      syntax: :proto3
    }
  end
end
