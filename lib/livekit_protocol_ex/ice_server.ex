...
defmodule LivekitProtocolEx.ICEServer do
  @moduledoc false
  ...
  defstruct credential: "", username: "", urls: [], __uf__: []
  ...
  (
    ...
    defp encode_credential({acc, acc_size}, msg) do
      if msg.credential == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.credential)
        {["\x1A", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:credential, "invalid field value"), __STACKTRACE__
    end
    ...
    defp encode_urls({acc, acc_size}, msg) do
      case msg.urls do
        [] ->
          {acc, acc_size}
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:urls, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.ICEServer))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<3::5, _wire_type::3, bytes::binary>> ->
              ...
              {[credential: Protox.Decode.validate_string!(delimited)], rest}
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[urls: msg.urls ++ [Protox.Decode.validate_string!(delimited)]], rest}
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.ICEServer,
      syntax: :proto3
    }
  end
end
