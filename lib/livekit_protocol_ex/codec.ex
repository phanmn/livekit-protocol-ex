...
defmodule LivekitProtocolEx.Codec do
  @moduledoc false
  ...
  defstruct fmtp_line: "", mime: "", __uf__: []
  ...
  (
    (
      ...
      def encode!(msg) do
        {_acc = [], _acc_size = 0}
        |> encode_fmtp_line(msg)
        |> encode_mime(msg)
        |> encode_unknown_fields(msg)
      end
    )

    defp encode_fmtp_line({acc, acc_size}, msg) do
      if msg.fmtp_line == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.fmtp_line)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:fmtp_line, "invalid field value"), __STACKTRACE__
    end

    defp encode_mime({acc, acc_size}, msg) do
      if msg.mime == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.mime)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:mime, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.Codec))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<2::5, _wire_type::3, bytes::binary>> ->
              ...
              {[fmtp_line: Protox.Decode.validate_string!(delimited)], rest}

            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[mime: Protox.Decode.validate_string!(delimited)], rest}
