...
defmodule LivekitProtocolEx.Psrpc.TopicParamOptions do
  @moduledoc false
  ...
  defstruct single_server: false, typed: false, names: [], group: "", __uf__: []
  ...
  (
    ...
    defp encode_single_server({acc, acc_size}, msg) do
      if msg.single_server == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.single_server)
        {[" ", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:single_server, "invalid field value"), __STACKTRACE__
    end

    defp encode_typed({acc, acc_size}, msg) do
      if msg.typed == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.typed)
        {["\x18", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:typed, "invalid field value"), __STACKTRACE__
    end
    ...
    defp encode_group({acc, acc_size}, msg) do
      if msg.group == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.group)
        {["\n", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.Psrpc.TopicParamOptions))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<1::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[group: Protox.Decode.validate_string!(delimited)], rest}
  ...
  (
    ...
    def default(:single_server) do
      {:ok, false}
    end

    def default(:typed) do
      {:ok, false}
    end
  ...
  def schema() do
    %{
      ...
      name: LivekitProtocolEx.Psrpc.TopicParamOptions,
      syntax: :proto3
    }
  end
end
