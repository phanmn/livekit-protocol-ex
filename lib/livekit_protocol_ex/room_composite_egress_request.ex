...
defmodule LivekitProtocolEx.RoomCompositeEgressRequest do
  @moduledoc false
  ...
  (
    ...
    defp encode_audio_mixing({acc, acc_size}, msg) do
      if msg.audio_mixing == :DEFAULT_MIXING do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.audio_mixing
          |> LivekitProtocolEx.AudioMixing.encode()
          |> Protox.Encode.encode_enum()
      ...
      ArgumentError ->
        reraise Protox.EncodingError.new(:audio_mixing, "invalid field value"), __STACKTRACE__
    end
    ...
    defp encode_custom_base_url({acc, acc_size}, msg) do
      if msg.custom_base_url == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.custom_base_url)
        {["*", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:custom_base_url, "invalid field value"), __STACKTRACE__
    end

    defp encode_video_only({acc, acc_size}, msg) do
      if msg.video_only == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.video_only)
        {[" ", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:video_only, "invalid field value"), __STACKTRACE__
    end

    defp encode_audio_only({acc, acc_size}, msg) do
      if msg.audio_only == false do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_bool(msg.audio_only)
        {["\x18", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:audio_only, "invalid field value"), __STACKTRACE__
    end

    defp encode_layout({acc, acc_size}, msg) do
      if msg.layout == "" do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} = Protox.Encode.encode_string(msg.layout)
        {["\x12", value_bytes | acc], acc_size + 1 + value_bytes_size}
      end
    rescue
      ArgumentError ->
        reraise Protox.EncodingError.new(:layout, "invalid field value"), __STACKTRACE__
    end
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.RoomCompositeEgressRequest))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<15::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_enum(bytes, LivekitProtocolEx.AudioMixing)
              {[audio_mixing: value], rest}
            ...
            <<5::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[custom_base_url: Protox.Decode.validate_string!(delimited)], rest}

            <<4::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_bool(bytes)
              {[video_only: value], rest}

            <<3::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_bool(bytes)
              {[audio_only: value], rest}

            <<2::5, _wire_type::3, bytes::binary>> ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[layout: Protox.Decode.validate_string!(delimited)], rest}
  ...
  (
    ...
    def default(:custom_base_url) do
      {:ok, ""}
    end

    def default(:video_only) do
      {:ok, false}
    end

    def default(:audio_only) do
      {:ok, false}
    end

    def default(:layout) do
      {:ok, ""}
    end
    ...
    %{
      ...
      fields: %{
        ...
        audio_mixing: %{
          __struct__: Protox.Field,
          extender: nil,
          kind: %{__struct__: Protox.Scalar, default_value: :DEFAULT_MIXING},
          label: :optional,
          name: :audio_mixing,
          tag: 15,
          type: {:enum, LivekitProtocolEx.AudioMixing}
        },
        audio_only: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.RoomCompositeEgressRequest,
      syntax: :proto3
    }
  end
end
