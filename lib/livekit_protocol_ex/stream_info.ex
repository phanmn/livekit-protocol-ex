...
defmodule LivekitProtocolEx.StreamInfo do
  @moduledoc false
  ...
  (
    ...
    defp encode_status({acc, acc_size}, msg) do
      if msg.status == :ACTIVE do
        {acc, acc_size}
      else
        {value_bytes, value_bytes_size} =
          msg.status
          |> LivekitProtocolEx.StreamInfo.Status.encode()
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(LivekitProtocolEx.StreamInfo))
        end
      )
    )
        ...
        {field, rest} =
          case bytes do
            ...
            <<5::5, _wire_type::3, bytes::binary>> ->
              {value, rest} = Protox.Decode.parse_enum(bytes, LivekitProtocolEx.StreamInfo.Status)
              {[status: value], rest}
    ...
    %{
      ...
      fields: %{
        ...
        status: %{
          ...
          name: :status,
          tag: 5,
          type: {:enum, LivekitProtocolEx.StreamInfo.Status}
        },
        url: %{
          __struct__: Protox.Field,
          extender: nil,
      ...
      name: LivekitProtocolEx.StreamInfo,
      syntax: :proto3
    }
  end
end
